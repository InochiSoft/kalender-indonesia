      ...
      for (let d = 0; d < 43; d++) {
        ...
        if (options.include?.remHolidays) {
          if (options.include?.remHolidays.length > 0) {
            if (resultDate.holiday) {
              for (const remHoliday of options.include?.remHolidays){
                if (resultDate.holiday.date === remHoliday.date) {
          ...
          if (options.include?.remLeaves.length > 0) {
            if (resultDate.leave) {
              for (const remLeave of options.include?.remLeaves){
                if (resultDate.leave.date === remLeave.date) {
        ...
        if (resultDate.holiday) {
          delete resultDate.holiday.date;
        }

        if (resultDate.leave) {
          delete resultDate.leave.date;
        }
